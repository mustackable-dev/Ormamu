...
internal static class Cache
{
        ...
        foreach (var type in AppDomain.CurrentDomain.GetAssemblies().SelectMany(x => x.GetTypes()))
        {
            ...
            if (configAttribute is not null && !buildOptions.TryGetValue(configAttribute.ConfigId, out options!))
            {
                throw new CacheBuilderException(
                    CacheBuilderExceptionType.ConfigNotFound,
                    configAttribute.ConfigId,
                    type.Name);
            }
            ...
            {
                typeMapConverter = x =>
                    customPropertyNameDictionary.TryGetValue(x, out string? value) ? value : options.NameConverter(x);
            }
    ...
    private static List<PropertyMapping> ConvertToCompositeKeyProperties(
        ...
        Type type)
    {
        PropertyInfo[] compositeKeyProperties =
            compositeKeyAttribute.KeyType.GetProperties(options.PropertyBindingFlags);
    ...
    private static PropertyData? GetPropertyData(PropertyInfo property)
    {
        ...
        {
            if (!property.PropertyType.GetElementType()!.IsValidType())
                return null;
        }
        else
        {
            if (!property.PropertyType.IsValidType())
                return null;
        }
